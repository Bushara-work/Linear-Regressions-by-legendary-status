...
def plot_regression_by_legendary_status(df):
    ...
    for i, row in df.iterrows():
        edge_color = get_generation_color(row)
        color = 'gold' if row['Legendary'] else 'silver'
        plt.scatter(row['Defense'], row['Attack'], color=color, edgecolor=edge_color, linewidth=1.2)
